	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>
{
	...
	pub fn process_deletion_queue_batch(max_weight: Weight) -> Weight {
		...
		let decoding_weight = weight_per_queue_item
			.saturating_mul(DeletionQueue::decode_len().unwrap_or(0) as Weight);
		let mut remaining_key_budget = max_weight
			.saturating_sub(T::WeightInfo::on_initialize())
		...
		max_weight.saturating_sub(weight_per_key * (remaining_key_budget as Weight))
	}
